...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
GPIO_TypeDef *led_rgb_ports[] = {LED_GREEN_GPIO_Port, LED_RED_GPIO_Port, LED_BLUE_GPIO_Port};
uint16_t led_rgb_pins[] = {LED_GREEN_Pin, LED_RED_Pin, LED_BLUE_Pin};
LED_RGB Led_rgb;
...
int main(void)
{
  ...
  MX_TIM8_Init();
  /* USER CODE BEGIN 2 */
	led_rgb_init(&Led_rgb, led_rgb_ports, led_rgb_pins);
	
	button_init(&Button_1, BUTTON_1_GPIO_Port, BUTTON_1_Pin);
	
	buzzer_init(&Buzzer, &htim8, TIM_CHANNEL_1);
	
	battery_init(&Battery, &hadc1);
