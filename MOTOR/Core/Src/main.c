...
int main(void)
{
  ...
  MX_TIM3_Init();
  /* USER CODE BEGIN 2 */
	motor_init(&MotorA, &htim1, TIM_CHANNEL_1, STSP_RST_GPIO_Port, STSP_RST_Pin, 
	PHA_GPIO_Port, PHA_Pin, PWMA_GPIO_Port, PWMA_Pin);
	motor_init(&MotorB, &htim1, TIM_CHANNEL_2, STSP_RST_GPIO_Port, STSP_RST_Pin, 
	PHB_GPIO_Port, PHB_Pin, PWMB_GPIO_Port, PWMB_Pin);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		set_speed(&MotorA, -500);
		set_speed(&MotorB, 500);
  }
  /* USER CODE END 3 */
}
